...
Done:
- BATS path portability — hardcoded `/home/YOUR_USERNAME/nixos-config/...` → `$(git rev-parse --show-toplevel)` in `test-frontmatter-valid.bats` + `test-statusline-format.bats`, matching `test-managed-settings.bats:14`.
...
- I4 — `test-phase4-validator.bats` Test 2 grep alternation only requires ONE exit-code branch found, not all 3 — tighten to assert all three present. Also: hardcoded `/home/YOUR_USERNAME/nixos-config/rebuild-nixos` path in same test should use `$BATS_TEST_DIRNAME` relative reference for portability.
